...
func newE2ERouter(t *testing.T) (*gin.Engine, *gorm.DB, config.AdminConfig) {
	...
	return Setup(tx, jwtCfg, corsCfg, adminCfg, config.PricingConfig{MinProgramPriceMinorUnits: 100}, config.CommissionConfig{DefaultPlatformCommissionBPS: 2000}, config.StripeConfig{}, config.PayPalConfig{}, config.WebhookConfig{}, config.TestModeConfig{}), tx, adminCfg
}
